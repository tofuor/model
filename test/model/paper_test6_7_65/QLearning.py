...
def SMA_plot_picture(RR_PacketLoss, BET_PacketLoss, DQN_PacketLoss, real_time_accumluation, tag):
    ...
    difference_rr = [(a - b) / a *100 for a, b in zip(RR_PacketLoss[min_length-1], DQN_PacketLoss[min_length-1])]
    difference_bet = [(a - b) / a * 100 for a, b in zip(BET_PacketLoss[min_length-1], DQN_PacketLoss[min_length-1])]
    real_time_rr = (real_time_accumluation[0] - real_time_accumluation[2]) / real_time_accumluation[0] * 100
    real_time_bet = (real_time_accumluation[1] - real_time_accumluation[2]) / real_time_accumluation[1] * 100
    ...
    # 第一張圖：UE1封包佇列累積量
    plt.figure(figsize=(10, 5))
    plt.plot(min_ticks, DQN_sma_ue1[:min_length], label='DQN Packet Queue Length', color='darkred')
    plt.plot(min_ticks, RR_sma_ue1[:min_length], label='RR Packet Queue Length', color='darkorange')
    plt.plot(min_ticks, BET_sma_ue1[:min_length], label='BET Packet Queue Length', color='olive')
    
    print(f"DQN_sma_ue1[:min_length] = {DQN_sma_ue1[:min_length]}")
    print(f"RR_sma_ue1[:min_length] = {RR_sma_ue1[:min_length]}")
    print(f"BET_sma_ue1[:min_length] = {BET_sma_ue1[:min_length]}")
    ...
    print(f"DQN_sma_ue2[:min_length] = {DQN_sma_ue2[:min_length]}")
    print(f"RR_sma_ue2[:min_length] = {RR_sma_ue2[:min_length]}")
    print(f"BET_sma_ue2[:min_length] = {BET_sma_ue2[:min_length]}")
...
class DQNAgent:
    def __init__(self):
        ...
        self.epsilon = 0.3874  # exploration rate
...
class PRBType():
    ...
    '''
    def __init__(self):
        self.video_PRB_utilization: list = [0,0]
        self.real_time_current: int = 0
        
    def reset(self, kpm_data:list, slice_select:list):
        self.accumulation: list = [0,0]
        self.real_time_current: int = 0
        
    def utilization_calculate(self, kpm_data:list, slice_select:list, max_base_station_throughput:int):
        '''
        ...
        '''
        video_max_throughput = max_base_station_throughput * slice_select[0]/(slice_select[0] + slice_select[1])
        video_PRB_utilization = kpm_data[0] / video_max_throughput
        real_time_max_throughput = max_base_station_throughput * slice_select[1]/(slice_select[0] + slice_select[1])
        real_time_PRB_utilization = kpm_data[1] / real_time_max_throughput
...
class Scheduler:
    ...
    def _step_display(self, kpm_data:list, iperf_data:list, packetloss:PacketLossType, reward:int, action:list):
        ...
        # PRB_utilization = video_throughput / max_throughput , 
        # max_throughput = max_base_station_throughput * video_slice/(video_slice + real_time_slice)
        # self.PRB_utilization.append()
        # self.PRB_utilization.append()
